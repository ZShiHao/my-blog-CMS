<script setup lang="ts">
   ...
   function validateUser() {
      if (validator.isEmail(email.value)) {
         type.value = 1;
         return true;
      } else if (validator.isMobilePhone(email.value, ['zh-CN'])) {
         type.value = 0;
         return true;
      } else {
         ElMessage({
...
<template>
   <section class="contain">
      <form class="form" action="">
         <input class="input" type="text" v-model="email" placeholder="Mobile Number or Email" />
      ...
      <button
         v-if="!if_register"
         :class="email.length >= 10 && password.length >= 8 ? 'button_active' : 'button_disable'"
         @click="loginHandler"
      >
         Log in
      </button>
      <button
         v-else
         :class="
            email.length >= 10 && password.length >= 8 && re_password.length === password.length
               ? 'button_active'
               : 'button_disable'
         "
